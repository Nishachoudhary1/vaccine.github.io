...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql1 = "SELECT * FROM user_credentials WHERE UserName ='".$username."' AND Password='".$password."'";
  ...
  while($userid = mysqli_fetch_array($uid)) {	
    ...
    $sql2 = "SELECT * FROM slotbookinginformation WHERE u_name ='".$username."'";
  $uid1 = mysqli_query($conn, $sql2);
  while($userid1 = mysqli_fetch_array($uid1)) {	

    $user_id1 = $userid1["u_name"];
